<template>
  <div>
        ...
        <b-card>
          <div class="d-md-flex justify-content-between align-items-center my-1">
            <div class="border-left-primary border-left-3">
              <p class="card-title p-50 mb-0">Master</p>
            </div>
            <div>
    ...
    <b-modal size="xl" :visible="showMasterModel" :title="MasterEdit ? 'Update Master' : 'Add Master'"
      ...
      <label class="form-label required">Master Name</label>
       <b-form-group  label-for="basicInput">
          <b-form-input id="basicInput" v-model="MasterData.name"   @input="RemoveError('name')"  placeholder="Enter Role Name" />
       ...
       <b-form-group  label-for="basicInput">
          <b-form-checkbox :value="MasterData.master_page" checked="1">Make Master Page</b-form-checkbox>
          <small class="text-danger">{{ errors[0] }}</small>
          <div class="text-danger" v-if="hasErrors('name')">
            {{ getErrors("name") }}
          </div>
        ...
        <div>
          <b-table :items="MasterData.masterFields" :fields="['title', 'type', 'required','show_list','show_filter', 'action']">
            ...
            <template #cell(action)="data">
              ...
              <b-button v-ripple.400="'rgba(255, 255, 255, 0.15)'" v-if="data.item.master_id != 0" variant="danger" pill @click="removeField(data.index)">
      ...
      >
        ...
        />
          ...
          <!-- <b-row v-for="field in MasterData.masterFields" :key="field.id" class="pt-1">
             {{field.validation_rule}}
          </b-row> -->
          <!-- <b-form-group label="Using sub-components:" v-slot="{ ariaDescribedby }">
            <b-form-checkbox-group
              id="checkbox-group-2"
              v-model="MasterData.masterFields"
              :aria-describedby="ariaDescribedby"
              name="flavour-2"
            >
              <b-form-checkbox v-for="field in MasterData.masterFields" :key="field.id" :value="field.id" checked="1">{{field.title}} - rules : {{field.validation_rule}}</b-form-checkbox>
            </b-form-checkbox-group>
          </b-form-group> -->
        </div>
        ...
        <!-- <b-form-group label="Team " label-for="basicInput">
          <v-select v-model="MasterData.team"
            :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
            :options="teamOptions"
            label="name"
            :reduce="(val) => val.id"
...
export default {
  ...
  methods: {
    ...
    Onshown() {
      ...
      if (this.MasterEdit != 0) {
        axios.get(`/master/${this.MasterEdit}/edit`).then((response) => {

          this.MasterData.name = response.data.data.title
          this.MasterData.id = response.data.data.id
          this.MasterData.masterFields = response.data.data.masterFields
        });
      } else {
        this.resetRoledata();
      }
  ...
  setup(props, { emit }) {
    const masterFields = ref([]);
    const submitRole = () => {
      if (MasterData.value.id != null) {
        axios.post(`/RoleUpdate/${MasterData.value.id}`, MasterData.value)
          .then(response => {
            refetchData()
        ...
        axios.post("/master", MasterData.value).then((response) => {
          refetchData();
    ...
    axios.get('getMasterFieldsBydefault').then(response =>  {
      BlankProjectData.masterFields = response.data.data;
      MasterData.value.masterFields = response.data.data;
    });

    const resetRoledata = () => { MasterData.value = JSON.parse(JSON.stringify(BlankProjectData))};
