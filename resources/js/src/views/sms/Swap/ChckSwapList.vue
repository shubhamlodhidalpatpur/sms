        ...
        <b-card>
          <div class="d-md-flex justify-content-between align-items-center my-1">
            <div class="border-left-primary border-left-3">
              <p class="card-title p-50 mb-0"> Check Swap</p>
            </div>
            <div>
            <b-row>
    </b-row>
        ...
        >
            ...
            <template #cell(request_date)="data">
              <div class="text-nowrap">
                <span
                  class="align-text-top text-capitalize"
                  :title="data.item.request_date"
                  >{{ data.item.request_date }}</span
                >
              </div>
            </template>
            <template #cell(mode)="data">
              <div class="text-nowrap">
                <span
                  class="align-text-top text-capitalize"
                  :title="data.item.mode"
                  >{{ data.item.mode }}</span
                >
              </div>
            </template>
                <template #cell(approver_name)="data">
              <div class="text-nowrap">
                <span
                  class="align-text-top text-capitalize"
                  :title="data.item.approver_name"
                  >{{ data.item.approver_name }}</span
                >
              </div>
            </template>
             ...
             <b-link
                v-if="data.item.status == 'rejected' || data.item.status == 'inprogress'"
                title="Approve"
              ...
              <b-link
                title="Reject"
                v-if="data.item.status == 'approved' || data.item.status == 'inprogress'"
...
export default {
  ...
  data() {
    return {
      ...
      hod: "",
      option: [""],
      fields: [{key: "name", sortable: true},{key: "request_date", sortable: true},{key: "mode", sortable: true},{key: "approver_name", sortable: true},'action'],
      EditId: 0,
      CheckInButton:false,
  ...
  setup(props, { emit }) {
    ...
    const fetchSwap = (ctx, callback) => {
     axios.get('CheckswapRequest', {
                 params: {
                    ...SearchFilter.value,
   ...
   const SearchFilter =ref(JSON.parse(JSON.stringify({ FilterData }))) ;
    ...
    const getErrors = (fieldName) => {
     ...
     const statusChange = (id, status) => {
      ...
      axios.post(`changeSwapRequestStatus/${id}/${status}`).then((response) => {
        refetchData();
