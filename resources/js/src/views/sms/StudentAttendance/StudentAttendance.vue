        ...
        <b-card>
          ...
          <div class="d-md-flex justify-content-between align-items-center my-1">
            <div class="border-left-primary border-left-3">
              <p class="card-title p-50 mb-0">Attendance</p>
            </div>
            <div>

              <div class="d-flex justify-content-between mt-2 mt-md-0">
                <b-button  variant="primary" class="mb-0 ml-md-1 basicButton"
          ...
          <b-table responsive  
           striped
            hover
            bordered stacked="sm" ref="refteamListTable" :items="fethMasterAttendance" :fields="fields"
            ...
            <template #cell(date)="data">
              <span class="text-nowrap">
                {{ data.item.attendance_date }}
              </span>
            </template>
            <template #cell(attendance_count)="data">
              <span class="text-nowrap">
                {{ data.item.total_attendance_count }}
              </span>
            </template>
             <template #cell(persent)="data">
              <span class="text-nowrap">
                {{ data.item.persent }}
              </span>
            </template>
            ...
            <template #cell(Action)="data">
              <div class="icon_flx">
                <b-button size="sm" v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="primary" class="mb-0 ml-1 basicButton"
                  :to="{ name: 'take-attendance' , params: { date: data.item.attendance_date,class: data.item.class_id } }">
                  Take
                </b-button>
                <!-- <b-button size="sm" v-ripple.400="'rgba(255, 255, 255, 0.15)'" variant="primary" class="mb-0 ml-1 basicButton"
                  @click="OpenModel()">
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        {key: 'attendance_count', sortable: true},
        {key: 'persent', sortable: true ,label:'%'},
        'Action',
      ],
      EmployeesData: [],
      DesignationOptions: [],
  ...
  setup(props, { emit }) {
    ...
    const fethMasterAttendance = (ctx, callback) => {
      axios.get('masterAttendances', {
        params: {
